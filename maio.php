...
for($i = 0; $i < $cont04; $i++){
        $ch1 = curl_init(
            'http://dadosabertos.almg.gov.br/ws/prestacao_contas/verbas_indenizatorias/deputados/'.$idDeputado[$i].'/2017/5?formato=json'
        );
        curl_setopt($ch1, CURLOPT_HTTPGET, true);
        curl_setopt($ch1, CURLOPT_RETURNTRANSFER, true);
        $jsonarray1 = curl_exec($ch1);
        $dataVerba[$i][5] = json_decode($jsonarray1, true);
    }
    curl_close($ch1);
    for($i = 0; $i < $cont04; $i++){
        $contador5 = @count($dataVerba[$i][5]['list']);
        for($j = 0; $j < $contador5; $j++){
            $valorVerba[$i][$j] = $dataVerba[$i][5]['list'][$j]['valor'];
        }
    }
    $select = $db->querySingle("SELECT idVerba FROM verbas WHERE mes = 'Maio'");
    ...
    <body>
        <div class="container">
            <div id="conteudo">
                    <h3> Esses são os 5 deputados que mais gastaram de suas verbas indenizatórias durante o mês de maio. </h3>
            <table>
            <tr>
            <th>ID</th>
            <th>Nome</th>
